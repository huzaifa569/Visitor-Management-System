...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const publicRoutes = ['/admin/training'];
  const isPublicRoute = publicRoutes.includes(pathname);

  const { user, isLoading, isAuthenticated } = useAuthGuard({
    requireAuth: !isPublicRoute, // ✅ Disable auth check for public routes
    allowedRoles: ['admin', 'manager', 'security'],
    redirectTo: '/login'
  });

  // Show loading while checking authentication (skip for public routes)
  if (isLoading && !isPublicRoute) { // ✅ Add condition
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500" aria-hidden="true"></div>
  ...
  // Don't render anything if not authenticated (skip for public routes)
  if (!isAuthenticated && !isPublicRoute) { // ✅ Add condition
    return null;
  }
